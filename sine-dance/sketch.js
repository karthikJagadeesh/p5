const app = p => {
  ...
  class Ball {
    ...
    show() {
      let color = [p.random(255), p.random(255), p.random(255)];
      p.noStroke();
      p.fill(color);
  ...
  p.setup = () => {
    let canvas = p.createCanvas(innerWidth - 50, innerHeight - 100);
    let noOfBalls = 40;
    let totalOffset = p.height / noOfBalls;
    for (let i = 0; i < noOfBalls; i++) {
      let offsetX = (i + 1) / 1000;
      let posYOffset = totalOffset * (i + 0.5);
      balls.push(new Ball(p.width / 2, posYOffset, 2, offsetX, i));
    }
  };
